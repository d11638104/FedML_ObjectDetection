...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, num_classes, img_dim=416, device=None):
    ...
    def compute_grid_offsets(self, grid_size, cuda=True, device=None):
    ...
    def forward(self, x, targets=None, img_dim=None, device=None):
        ...
        if grid_size != self.grid_size:
            self.compute_grid_offsets(grid_size, cuda=x.is_cuda, device=device)
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=416, args=None):
    ...
    def forward(self, x, targets=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                ...
                x, layer_loss = module[0](x, targets, img_dim, self.device)
                loss += layer_loss
                yolo_outputs.append(x)
            layer_outputs.append(x)
        yolo_outputs = to_cpu(torch.cat(yolo_outputs, 1))
        return (0, yolo_outputs) if targets is None else (loss, yolo_outputs)
