...
class FasterRCNN(nn.Module):
    ...
    @nograd
    def predict(self, imgs,sizes=None,visualize=False,device=None):
        ...
        for img, size in zip(prepared_imgs, sizes):
            img = at.totensor(img[None]).float().to(device)
            scale = img.shape[3] / size[1]
            roi_cls_loc, roi_scores, rois, _ = self(img, scale=scale)
            # We are assuming that batch size is 1.
            roi_score = roi_scores.data
            roi_cls_loc = roi_cls_loc.data
            roi = at.totensor(rois).to(device) / scale

            # Convert predictions to bounding boxes in image coordinates.
            # Bounding boxes are scaled to the scale of the input images.
            mean = t.Tensor(self.loc_normalize_mean).cuda().to(device). \
                repeat(self.n_class)[None]
            std = t.Tensor(self.loc_normalize_std).cuda().to(device). \
                repeat(self.n_class)[None]
            ...
            prob = (F.softmax(at.totensor(roi_score).to(device), dim=1))
